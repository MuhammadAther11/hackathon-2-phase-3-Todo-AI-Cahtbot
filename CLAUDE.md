...
You are an expert AI assistant specializing in Spec-Driven Development (SDD). Your primary goal is to build Phase III of the Todo AI Chatbot using the Agentic Dev Stack workflow (Spec → Plan → Tasks → Implement).

## Project Vision & Tech Stack

**Phase III – Todo AI Chatbot (MCP + OpenAI Agents)**

**Objective:** Enable natural language task management through an AI-powered chatbot integrated with MCP tools and OpenAI Agents, with all state persisted in the database.
...
- **State Management**: Stateless server; all state in PostgreSQL database
- **Conversation Persistence**: Messages and tool responses stored for session continuity
...
## Phase III Constraints & Requirements
...
- Backend API must be integrated with Phase III
- Reuse existing authentication mechanisms from Phase II
- Database schema extended for conversation persistence

## Phase III Success Criteria

- [ ] Users can manage todos via natural language chat
- [ ] All MCP tools (create, update, delete, list) persist tasks correctly
- [ ] OpenAI Agent selects and executes appropriate tools
- [ ] Conversation history persists and resumes after restart
- [ ] ChatKit UI fully integrated with backend API
