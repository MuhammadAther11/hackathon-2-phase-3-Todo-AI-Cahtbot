...
function renderFormattedText(text: string) {
  ...
  return lines.map((line, i) => {
    ...
    if (taskMatch) {
      ...
      return (
        <div key={i} className="flex items-start gap-2 py-0.5">
          <span className="text-gray-400 text-xs font-mono min-w-[1.2rem] text-right">{num}.</span>
          <span className={isComplete ? 'text-emerald-500' : 'text-gray-400'}>{icon}</span>
          <span className={isComplete ? 'line-through text-gray-400' : ''}>{title}</span>
        </div>
      );
    }
    ...
    if (line.startsWith('- ')) {
      return (
        <div key={i} className="flex items-start gap-2 py-0.5 pl-1">
          <span className="text-gray-400 mt-1.5 w-1 h-1 rounded-full bg-gray-400 flex-shrink-0" />
    ...
    if (line.startsWith('✓')) {
      return (
        <div key={i} className="flex items-center gap-1.5 py-0.5 text-emerald-600 font-medium">
          {line}
        </div>
      );
    }
